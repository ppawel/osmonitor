...
class PG_DB
  ...
  WAY_GEOM_SQL = \
    'SELECT way_id AS id, sequence_id, node_id 
     FROM way_nodes 
     WHERE way_id = %{id};'
  RELATION_GEOM_SQL = \
    'SELECT relation_id AS id, sequence_id, member_type, member_id, member_role 
  ...
  TAGS_SQL = \
    'SELECT %{type}_id AS id, k, v 
     FROM %{type}_tags
     WHERE %{type}_id = %{id};'
  HISTORY_SQL = \
    'SELECT %{type}s.%{type}_id AS id, %{type}s.version AS changeset_id, visible %{fields}
     FROM %{type}s 
