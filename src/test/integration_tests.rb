...
class IntegrationTests < Test::Unit::TestCase
  ...
  def test_y_shaped_road
    ...
    assert_equal(1, status.road.comps.size)
    #assert_equal(2, status.road.comps[0].paths.size)
    assert_equal(155.0, road.comps[0].dist(road.get_node(3), road.get_node(4)))
    assert_equal(210.0, road.comps[0].dist(road.get_node(5), road.get_node(1)))
    assert_equal(road.get_node(1), road.comps[0].furthest(road.get_node(5)))
    assert_equal(road.get_node(5), road.comps[0].furthest(road.get_node(3)))
    assert_equal(road.get_node(3), road.comps[0].closest_end_nodes(road.get_node(3))[0])
    assert_equal(road.get_node(3), road.comps[0].closest_end_nodes(road.get_node(1))[1])
  ...
  def test_dw255
    ...
    assert(!status.has_issue_by_name?('relation_disconnected'))
    puts road.comps[0].end_nodes.inspect
    #assert_equal(road.get_node(259982309), road.comps[0].furthest(road.get_node(683182935)))
    #assert(!road.length.nil?)
  end
  ...
  def test_dw796
    ...
    def road_manager.load_ways(road)
      road_data_from_file('road_data_DW796.txt')
    end

    input = RoadInput.new
    road = road_manager.load_road('DW', '796')
    status = RoadStatus.new(input, road)
    status.validate
