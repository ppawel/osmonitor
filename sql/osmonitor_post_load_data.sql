...
CREATE FUNCTION OSM_Preprocess() RETURNS void AS $$
	-- Fill the way_nodes#geom column
	UPDATE way_nodes wn
	SET geom = (SELECT n.geom FROM nodes n WHERE n.id = wn.node_id)
	WHERE wn.way_id IN (SELECT w.id FROM ways w WHERE w.tstamp > OSM_GetConfigDateValue('last_preprocessing_data_timestamp'));

	-- Fill the way_nodes#dist_to_next column
	UPDATE way_nodes wn SET dist_to_next = ST_Distance_Sphere(wn.geom,
		(SELECT
			wn_next.geom
	...
	SET refs = 
		...
		END)
	WHERE tags ? 'ref' AND w.tstamp > OSM_GetConfigDateValue('last_preprocessing_data_timestamp');
	
	UPDATE config_options SET date_value = OSM_GetDataTimestamp() WHERE option_key = 'last_preprocessing_data_timestamp';
$$ LANGUAGE SQL;
